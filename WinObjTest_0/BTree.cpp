...
template <class T> inline T* BTree<T>::Find(T* pValue)
{
  int       nCompRc;
  BTree<T>* pTemp   = NULL;
  BTree<T>* pBase   = NULL;
  ...
  if ( pValue )
      ...
      do
	{
      ...
      pRet = (NULL != pBase ? pBase->GetValue() : NULL);
...
{
#if defined(_MT)
  BTree<T>* pValue = new BTree<T>(pNewValue,m_cs);
#else
  BTree<T>* pValue = new BTree<T>(pNewValue);
#endif
  BTree<T>* pRetVal;

  pRetVal = Insert(pValue);

  if ( NULL == pRetVal ) delete pValue;
...
template <class T> T** BTree<T>::GetSortedItemList()
{
  ...
  for( i=0 ; i < nItemCount ; ++i ) {
    pItemList[i] = pSortList[i]->GetValue();
  }
